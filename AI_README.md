...
# Suggested Frontend AI Integration Plan
AI Module | Integration Location & UI Behavior
----|----
AI Symptom Checker (LLM) | Landing Page -> Features Section:<br>Modify 3 Simple Steps text to link to a Symptom Assessment Modal.<br>Patient Dashboard -> Sidebar:<br>Add Symptom Check item below Find Doctors that opens a dedicated triage chat interface.
Ask Clinic Chatbot (RAG) | Global (All Pages) -> Bottom Right:<br>Add a Floating Action Button (FAB) with a chat icon.<br>UI Behavior:<br>Clicking expands a chat widget overlay for instant Q&A (hours, insurance, location).
Doctor Recommender | Landing Page -> Hero Section:<br>Add secondary button Help Me Find a Doctor next to Register.<br>UI Behavior:<br>Launches a Step-by-Step Wizard (not chat) that asks 3-4 questions and auto-applies filters to the Find Doctors page.
Sentiment Analysis | Doctor Dashboard -> Stats Row:<br>Add a 4th card labeled Patient Satisfaction.<br>UI Behavior:<br>Displays a score (e.g., 4.8/5.0) and trend. Clicking navigates to a Reviews Analytics detail page.
Content Moderation | Messages Page -> Chat Interface:<br>Runs silently in the background.<br>UI Behavior:<br>If content is flagged, replace the message bubble with gray italic text: [Message hidden due to inappropriate content].
Image Recognition (OCR) | Patient Management -><br> Add Patient Modal:Add a Scan ID / Insurance Card button to the top of the form.<br>UI Behavior:<br>Clicking opens the camera/file picker. The form fields (Name, ID #) auto-populate after processing.
Voice Search | Appointments Page -> Search Bar:<br>Add a Microphone Icon inside the search input.<br>UI Behavior:<br>Clicking indicates recording (red border); speech is converted to text in the input box in real-time.

# Short Term To-DO List
- Database for the llm responses? (chat history)
